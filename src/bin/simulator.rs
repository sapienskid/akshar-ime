...
fn get_log_path() -> PathBuf {
    ...
    path.push("nepali_ime_rust_simulator.log");
    path
}
...
fn main() -> io::Result<()> {
    ...
    log("--- Nepali IME Rust Engine Simulator Starting ---");
    ...
    for line in stdin.lock().lines() {
        ...
        match command {
            "PROCESS_KEY_EVENT" => {
                ...
                if handle_key_event(key_val, &mut preedit, &mut engine, &mut stdout)? {
...
fn commit_preedit(preedit: &mut String, engine: &mut ImeEngine, stdout: &mut io::Stdout) -> io::Result<()> {
    if !preedit.is_empty() {
        let suggestions = engine.get_suggestions(preedit, 1);
        let to_commit = if !suggestions.is_empty() {
            suggestions[0].0.clone()
        } else {
            engine.romanizer.generate_candidates(preedit).get(0).cloned().unwrap_or_else(|| preedit.clone())
        };
...
fn handle_key_event(key_val: u32, preedit: &mut String, engine: &mut ImeEngine, stdout: &mut io::Stdout) -> io::Result<bool> {
    let mut needs_update = true;
    if let Some(c) = std::char::from_u32(key_val) {
        match key_val {
            // --- CHANGED: Space and Enter are now identical pure commit keys ---
            32 | 65293 | 65289 => { // Space, Enter, Tab
                commit_preedit(preedit, engine, stdout)?;
                // Preedit is now empty, so the UI needs to update to hide suggestions.
                needs_update = true;
            }
            ...
            _ => {
                if c.is_ascii_alphabetic() {
                    ...
                    let nepali_symbol = engine.romanizer.generate_candidates(&char_str).get(0).cloned().unwrap_or(char_str);
                    let cmd = format!("COMMIT_TEXT {}", nepali_symbol);
                    log(&format!("Rust -> '{:?}'", cmd));
                    writeln!(stdout, "{}", cmd)?;
                    needs_update = false; 
                } else {
...
fn update_ibus(preedit: &str, suggestions: &[(String, u64)],  stdout: &mut io::Stdout) -> io::Result<()> {
    ...
    if suggestions.is_empty() && preedit.is_empty() {
