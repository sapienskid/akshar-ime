    ...
    fn generate_edits(&self, word: &str) -> HashSet<String> {
        ...
        for _ in 0..self.max_edit_distance {
            ...
            for edit in current_edits {
                // Delete by Unicode scalar position, not raw bytes, so Indic script
                // strings do not panic on non-ASCII character boundaries.
                for (i, _) in edit.char_indices() {
...
mod tests {
    ...
    #[test]
    fn symspell_handles_devanagari_without_utf8_panics() {
        let mut symspell = SymSpell::new(2);
        symspell.add_word("राम", 1);
        let matches = symspell.lookup("राम");
