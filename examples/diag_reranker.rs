...
use akshar_ime::core::reranker::{extract_dense_features, extract_sparse_features, rank_candidates, FreqRanks, DENSE_DIM};
...
fn main() {
    ...
    println!("slots {} (HASH_SIZE {}), non-zero {} ({:.4}%), scale {:.3e}",
             t.len(), HASH_SIZE, nz, nz as f64 / t.len().max(1) as f64 * 100.0, u.sparse_scale);
    ...
    println!("\n=== replaying reranker training on {} valid pairs ===", pairs.len());

    struct Item { target_idx: usize, base: Vec<f64>, sparse: Vec<Vec<usize>> }
    ...
    for (roman, gold) in &pairs {
        ...
        if let Some(target_idx) = order.iter().position(|c| &c.dev == gold) {
            let sparse: Vec<Vec<usize>> = order.iter().map(|c| {
                let aks = akshar_ime::core::akshara::segment(&c.dev);
                extract_sparse_features(&c.dev, roman, c.akshara_count, &aks)
            }).collect();
            if sparse.iter().any(|s| s.is_empty()) { empty_sparse += 1; }
            if sparse.len() > 1 && sparse.iter().all(|s| *s == sparse[0]) { identical_sparse += 1; }
            let base: Vec<f64> = order.iter().enumerate().map(|(idx, c)| {
                let dense = extract_dense_features(c, idx, heur[idx], heur_rank[idx], roman, &vocab_freq, &ranks);
                (0..DENSE_DIM).map(|k| W_DENSE[k] * ((dense[k] - MEAN_DENSE[k]) / STD_DENSE[k])).sum()
            }).collect();
            samples.push(Item { target_idx, base, sparse });
        }
    }
    println!("samples with gold in top-50: {}", samples.len());
    println!("samples where some candidate has NO sparse features: {}", empty_sparse);
    println!("samples where ALL candidates share identical sparse features: {}", identical_sparse);
    ...
    for ep in 1..=8 {
        ...
        for s in &samples {
            ...
            for (idx, sf) in s.sparse.iter().enumerate() {
                for &h in sf { scores[idx] += f64::from(table[h]); }
            }
            ...
            if probs.iter().enumerate().max_by(|a,b| a.1.partial_cmp(b.1).unwrap()).is_some_and(|(i,_)| i == s.target_idx) { hits += 1; }
        ...
        println!("  epoch {}: loss={:.4} top1={:.2}%  lr={:.2e}  nonzero_slots={}",
                 ep, loss / samples.len() as f64, hits as f64 / samples.len() as f64 * 100.0, lr, nz);
        lr *= 0.8;
    }
}
