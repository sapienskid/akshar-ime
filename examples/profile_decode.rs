...
fn main() {
    ...
    let (_, t_trie) = phase!("decode_in_words_detailed (trie beam)", {
        let mut acc = 0usize;
        for q in &qs {
            acc += dec.decode_in_words_detailed(q, 50, &trie).len();
        }
        acc
    });
    let (cands, t_union) = phase!("decode_union (both + merge)", {
        let mut v = Vec::new();
        for q in &qs {
            v.push(dec.decode_union(q, 50, Some(&trie)));
        }
        v
    });
    ...
    let (_, t_all) = phase!("ImeEngine::get_suggestions (end to end)", {
        let mut acc = 0usize;
        for q in &qs {
            acc += engine.get_suggestions(q, 5).len();
        }
        acc
    });
